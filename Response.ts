...
export class Response {
  ...
  public setCookies: {[_: string]: {value: string, options: CookieOptions}|null} = {};
  ...
  public setCookie(key: string, value: string, options: CookieOptions = {}): Response {
  ...
  public async _respond(): Promise<void> {
    ...
    if (this.body !== null) {
      ...
      await this.ctx.req.original.respond({ body, headers: new Headers([['content-type', this.mimeType], ...cookieHeaders, ...this.headers]), status: this.status });
      this._sent = true;
    } else {
